...
class WC_ScanForPay extends WC_Payment_Gateway{	
	...
	public function __construct(){
		$this->id = "scanforpay"; // 你的网关的唯一ID，例如，“your_gateway”
		$this->icon = "https://pay.scanforpay.com/static/plugin/alipay.svg";
		//如果你想要付款字段在结算页面显示，就设置为true（如果做的直接集成） 
		$this->has_fields = false;
		//显示在后台的付款方式的标题
		$this->method_title = 'ScanForPay';
		//显示在后台的付款方式的描述。
		...
		ScanForPay_API::set_partnerNo($this->partnerNo);
		ScanForPay_API::set_sandbox($this->sandbox);
		ScanForPay_API::set_storeNo($this->merchantId);
		ScanForPay_API::set_signKey($this->signKey);
		ScanForPay_API::set_paymentType($this->paymentType);
		// ScanForPay_API::set_wallet("Alipay");

		self::$log_enabled    = $this->sandbox;
	...
	function process_payment( $order_id ) {
		...
		$result = ScanForPay_API::create_order($order, 'Alipay');
